...
class Main(object):
    ...
    def __init__(self):
        ...
        self.tf_list_50 = list(
            itertools.islice(sorted(self.tf_list.items(), key=operator.itemgetter(1), reverse=True), 50))
        tf_list_obj = TxtOut(self.tf_list_file_name, self.tf_list_50)  # csv output for tf
        tf_wordCloud_obj = WordCloudOut(self.tf_wordCloud_file_name, self.tf_wordCloud_title,
                                        self.tf_list_50)  # word cloud for tf
        ...
        self.tfidf_list_50 = list(
            itertools.islice(sorted(self.tfidf_list.items(), key=operator.itemgetter(1), reverse=True), 50))
        tfidf_list_obj = TxtOut(self.tf_idf_list_file_name, self.tfidf_list_50)  # csv output for tf-idf
        tfidf_wordCloud_obj = WordCloudOut(self.tf_idf_wordCloud_file_name, self.tf_idf_wordCloud_title,
                                           self.tfidf_list_50)  # word cloud for tf-idf

    # get all file names from defined path
    def createFileList(self):
        path = "/home/erhan/PycharmProjects/TxTMining/"
